...
public class MainActivity extends Activity {
    ...
    private boolean bridgeMicPermissionPending;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        configureWebView();
        webView.addJavascriptInterface(new AppBridge(), "BuxoroNative");
    ...
    private void configureWebView() {
        ...
        settings.setUserAgentString(settings.getUserAgentString() + " BuxoroTibbiyotApp/2.3.2");
    ...
    private class AppBridge {
        @JavascriptInterface public void requestMicrophone() {
            runOnUiThread(() -> {
                if (checkSelfPermission(Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) { sendMicrophonePermissionResult(true); return; }
                bridgeMicPermissionPending = true;
                requestPermissions(new String[]{Manifest.permission.RECORD_AUDIO}, MICROPHONE_PERMISSION_REQUEST);
            });
        }
    }

    private void sendMicrophonePermissionResult(boolean granted) {
        if (webView == null) return;
        webView.evaluateJavascript("window.__buxoroMicPermissionResult&&window.__buxoroMicPermissionResult(" + (granted ? "true" : "false") + ")", null);
    }
    ...
    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == MICROPHONE_PERMISSION_REQUEST) {
            ...
            if (bridgeMicPermissionPending) { bridgeMicPermissionPending = false; sendMicrophonePermissionResult(granted); }
