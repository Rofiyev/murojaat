...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        webView = findViewById(R.id.webView); progress = findViewById(R.id.progress); offlineView = findViewById(R.id.offlineView);
        Button retryButton = findViewById(R.id.retryButton);
        try { allowedOrigin = new URI(APP_SERVER); } catch (URISyntaxException ignored) { }
        configureWebView(); retryButton.setOnClickListener(v -> loadServer()); ensureWatchSchedule(); loadServer();
    }
    ...
    private void configureWebView() {
        ...
        settings.setMixedContentMode(WebSettings.MIXED_CONTENT_NEVER_ALLOW); settings.setSavePassword(false); settings.setSupportZoom(false); settings.setGeolocationEnabled(true);
        settings.setUserAgentString(settings.getUserAgentString() + " BuxoroTibbiyotApp/2.2"); if (Build.VERSION.SDK_INT >= 26) settings.setSafeBrowsingEnabled(true);
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public void onGeolocationPermissionsShowPrompt(String origin, GeolocationPermissions.Callback callback) {
                ...
                pendingGeoCallback = callback; pendingGeoOrigin = origin; requestPermissions(new String[]{Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION}, LOCATION_PERMISSION_REQUEST);
            }
            @Override public void onPermissionRequest(PermissionRequest request) {
                runOnUiThread(() -> {
                    ...
                    boolean wantsAudio = false; for (String resource : request.getResources()) if (PermissionRequest.RESOURCE_AUDIO_CAPTURE.equals(resource)) { wantsAudio = true; break; }
                    ...
                    pendingMicRequest = request; requestPermissions(new String[]{Manifest.permission.RECORD_AUDIO}, MICROPHONE_PERMISSION_REQUEST);
                });
            }
    ...
    private boolean handleNavigation(Uri uri) {
        ...
        try { startActivity(new Intent(Intent.ACTION_VIEW, uri)); } catch (ActivityNotFoundException ignored) { Toast.makeText(this, "Havolani ochib bo‘lmadi.", Toast.LENGTH_SHORT).show(); } return true;
    }
    private void registerStatusWatch(String reference, String phone) {
        if (reference == null || phone == null) return; reference = reference.trim().toUpperCase(); phone = phone.trim(); if (reference.length() < 8 || phone.length() < 7) return;
        android.content.SharedPreferences prefs = getSharedPreferences(WATCH_PREFS, MODE_PRIVATE); Set<String> refs = new HashSet<>(prefs.getStringSet("refs", new HashSet<>())); refs.add(reference);
        prefs.edit().putStringSet("refs", refs).putString("phone_" + reference, phone).putString("status_" + reference, "yangi").apply(); ensureWatchSchedule();
        WorkManager.getInstance(this).enqueue(new OneTimeWorkRequest.Builder(StatusWorker.class).setConstraints(networkConstraints()).build()); requestNotificationPermissionIfNeeded(); Toast.makeText(this, "Holat o‘zgarsa ilova xabar beradi.", Toast.LENGTH_SHORT).show();
    }
    ...
    private void ensureWatchSchedule() {
        ...
        PeriodicWorkRequest request = new PeriodicWorkRequest.Builder(StatusWorker.class, 15, TimeUnit.MINUTES).setConstraints(networkConstraints()).build(); WorkManager.getInstance(this).enqueueUniquePeriodicWork(WATCH_WORK, ExistingPeriodicWorkPolicy.KEEP, request);
    }
    private void requestNotificationPermissionIfNeeded() { if (Build.VERSION.SDK_INT >= 33 && checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS}, NOTIFICATION_PERMISSION_REQUEST); }
    ...
    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == LOCATION_PERMISSION_REQUEST && pendingGeoCallback != null) { boolean granted = false; for (int result : grantResults) if (result == PackageManager.PERMISSION_GRANTED) { granted = true; break; } pendingGeoCallback.invoke(pendingGeoOrigin, granted, false); pendingGeoCallback = null; pendingGeoOrigin = null; }
        if (requestCode == MICROPHONE_PERMISSION_REQUEST && pendingMicRequest != null) { boolean granted = grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED; if (granted) pendingMicRequest.grant(new String[]{PermissionRequest.RESOURCE_AUDIO_CAPTURE}); else pendingMicRequest.deny(); pendingMicRequest = null; }
    }
    @Override @SuppressWarnings("deprecation") protected void onActivityResult(int requestCode, int resultCode, Intent data) { super.onActivityResult(requestCode, resultCode, data); if (requestCode == FILE_CHOOSER_REQUEST && fileCallback != null) { Uri[] results = WebChromeClient.FileChooserParams.parseResult(resultCode, data); fileCallback.onReceiveValue(results); fileCallback = null; } }
    ...
    @Override protected void onDestroy() { if (fileCallback != null) { fileCallback.onReceiveValue(null); fileCallback = null; } if (pendingGeoCallback != null) { pendingGeoCallback.invoke(pendingGeoOrigin, false, false); pendingGeoCallback = null; } if (pendingMicRequest != null) { pendingMicRequest.deny(); pendingMicRequest = null; } if (webView != null) { webView.stopLoading(); webView.setWebChromeClient(null); webView.setWebViewClient(null); webView.destroy(); } super.onDestroy(); }
}
